...
def load_data(X_train, X_features_train, y_train, X_val, X_features_val, y_val, device):
...
def model_setup(X_features_train, le, device):    
    criterion = nn.CrossEntropyLoss()
    scaler = torch.amp.GradScaler('cuda')
    model = RespiratoryFusionModel(num_classes=len(le.classes_), num_features=X_features_train.shape[1]).to(device)

    best_acc = 0.0
    history = {"train_loss": [], "val_acc": []}

    phases = [{"name": "Warmup (Freeze)", "epochs": 10, "lr": 1e-3, "freeze": True},
...
def get_predictions(model, X_test, X_features_test, y_test, augmenter, device):
